...
const TaskConfigPage = (props) => {
  ...
  const [currentColor, setCurrentColor] = useState(
    colorThemes.indexOf(defaultTask.theme)
  );
  ...
  useEffect(() => {
    ...
    picmoEmojiPicker = createPopup(
      ...
      {
        referenceElement:
          emojiContainer.current || document.querySelector("App"),
        triggerElement: emojiContainer.current,
        onPositionLost: "close",
      }
    );

    picmoEmojiPicker.addEventListener("emoji:select", (EmojiSelection) =>
      saveEmoji(EmojiSelection)
    );
  });
    ...
    <div className="task-config-page">
      ...
      <div className="task-config-form">
        <div className="task-config-form__field">
          ...
          <input
            type="text"
            value={props.previousChoices.buildOrQuit}
            readOnly
          ></input>
        </div>
        ...
        <div className="task-config-form__field">
          <label>PERIOD</label>
          <div
            className="task-config-form__dropdown form-input"
            onClick={togglePeriodDropdown}
          >
            {taskFrequency}
            {periodDropdownDisplay === "none" ? (
              <span className="task-config-form__dropdown-icon fa-solid fa-chevron-down"></span>
            ) : (
              <span className="task-config-form__dropdown-icon fa-solid fa-chevron-up"></span>
            )}
            <ul
              className="task-config-form__dropdown-content"
              style={{ display: periodDropdownDisplay }}
            >
              <li
                onClick={(event) => {
                  changeFrequency("Every day");
                }}
              >
                Every day
              </li>
              ...
              <li
                onClick={(event) => {
                  changeFrequency("Select week days");
                }}
              >
                Select week days
              </li>
        ...
        {taskFrequency === "Select week days" ? (
          <WeekdaySelector
            saveSelectedDays={saveSelectedDaysHandler}
            loadWeekdays={defaultTask.weekdays}
          />
        ) : (
          ""
        )}
        ...
        {taskFrequency === "Every X days" ? (
          <div className="task-config-form__field">
            ...
            <input
              ...
              max={6}
              value={taskOccurence}
              onChange={(e) => handleOccurenceInput(e)}
            ></input>
          </div>
        ) : (
          ""
        ...
        <div className="task-config-form__emoji-colour-container">
          <div className="task-config-form__field emoji">
            <label>EMOJI</label>
            <div
              className="task-config-form__emoji-picker"
              onClick={openEmojiPicker}
              ref={emojiContainer}
            >
              <span className="task-config-form__emoji-picker-emoji">
                {taskEmoji}
              </span>
            </div>
          </div>
          ...
          <div className="task-config-form__field color">
            <label>COLOR</label>
            <div
              className="task-config-form__color-picker"
              style={{
        ...
        {props.thisIsAnEdit ? (
          <button className="task-config-form__submit" onClick={saveTaskEdits}>
            Save edits
          </button>
        ) : (
          <button className="task-config-form__submit" onClick={saveTask}>
            Create Task
          </button>
        )}
      </div>
